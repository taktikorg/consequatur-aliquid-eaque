# @taktikorg/consequatur-aliquid-eaque&nbsp;&nbsp;[![Build Status](https://github.com/taktikorg/consequatur-aliquid-eaque/workflows/CI/badge.svg)](https://github.com/taktikorg/consequatur-aliquid-eaque/actions)&nbsp;[![Coverage Status](https://coveralls.io/repos/github/pinojs/@taktikorg/consequatur-aliquid-eaque/badge.svg?branch=master)](https://coveralls.io/github/pinojs/@taktikorg/consequatur-aliquid-eaque?branch=master)
...
```
npm install @taktikorg/consequatur-aliquid-eaque -g
```

## Usage

```
  @taktikorg/consequatur-aliquid-eaque

  To send pino logs to elasticsearch:

     cat log | @taktikorg/consequatur-aliquid-eaque --node http://localhost:9200
...
const pinoElastic = require('@taktikorg/consequatur-aliquid-eaque')
...
The stream handler returned from the default factory function is an [`EventEmitter`](https://nodejs.org/api/events.html#events_class_eventemitter).  Developers can use this interface to debug issues encountered by the `@taktikorg/consequatur-aliquid-eaque` library.
...
| ----- | ------------------ | ----------- |
| `unknown` | `(line: string, error: string) => void` | Event received by `@taktikorg/consequatur-aliquid-eaque` is unparsable (via `JSON.parse`) |
...
```
cat log | @taktikorg/consequatur-aliquid-eaque --node https://user:pwd@localhost:9200
```

Alternatively you can supply a combination of `username` and `password` OR `api-key`:
```
cat log | @taktikorg/consequatur-aliquid-eaque --node https://localhost:9200 -u user -p pwd
```
```
cat log | @taktikorg/consequatur-aliquid-eaque --node https://localhost:9200 --api-key=base64EncodedKey
```

Elastic cloud option `cloud` is also supported:
```sh
cat log | @taktikorg/consequatur-aliquid-eaque --cloud=name:bG9jYWxob3N0JGFiY2QkZWZnaA== --api-key=base64EncodedKey
```
...
use @taktikorg/consequatur-aliquid-eaque as a module is simple, use [pino-multi-stream](https://www.npmjs.com/package/pino-multi-stream) to send log to multi transport, for example:

```js
const pinoms = require('pino-multi-stream')
const pinoEs = require('@taktikorg/consequatur-aliquid-eaque')({
...
Setting up @taktikorg/consequatur-aliquid-eaque is easy, and you can use the bundled
...
You can test it by launching `node example | @taktikorg/consequatur-aliquid-eaque`, in
this project folder. You will need to have `@taktikorg/consequatur-aliquid-eaque`
installed globally.
